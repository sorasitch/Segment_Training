...
def get_model1_mod(img_size, num_classes):
    inputs = keras.Input(shape=img_size + (3,))
...
if os.path.exists('oxford_segmentation_color_r1.h5') and os.path.isfile('oxford_segmentation_color_r1.h5') :
    pass
    # load
    model=keras.models.load_model('oxford_segmentation_color_r1.h5',compile=False)
    # model.summary()
    ...
    adam = keras.optimizers.Adam() # tf.keras.optimizers.Adam()
    model.compile(optimizer=adam, loss="mae")
    
    callbacks = [
        keras.callbacks.ModelCheckpoint("oxford_segmentation_color_r1.h5", save_best_only=True)
    ]
...
with tf.device("CPU"):
    ...
    print(val_gen[10][0][1].shape)
    display(img1)
    img1.save('./out/oxford_segmentation_color_r1_2.png')
    img1 = PIL.ImageOps.autocontrast(keras.preprocessing.image.array_to_img(val_gen[10][1][1]))
    print(val_gen[10][1][1].shape)
    display(img1)
    img1.save('./out/oxford_segmentation_color_r1_3.png')
...
def display_mask(i):
    ...
    display(img)
    img.save('./out/oxford_segmentation_color_r1_4.png')
    ...
    display(img)
    img.save('./out/oxford_segmentation_color_r1_5.png')
    ...
    for j in range(val_preds[i].shape[-1]):
        ...
        display(img)
        img.save('./out/oxford_segmentation_color_r1_5_%d.png'%(j))
...
display(img)
img.save('./out/oxford_segmentation_color_r1_9.png')
