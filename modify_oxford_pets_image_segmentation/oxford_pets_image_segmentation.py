...
if os.path.exists('oxford_segmentation.h5') and os.path.isfile('oxford_segmentation.h5') :
    pass
    # load
    model=keras.models.load_model('oxford_segmentation.h5',compile=False)
    # model.summary()
...
def display_mask(i):
    ...
    img = PIL.ImageOps.autocontrast(keras.preprocessing.image.array_to_img(val_preds[i][:,:,:3]))
    display(img)
    for j in range(val_preds[i].shape[2]):
...
val_gen = OxfordPets(batch_size, img_size, train_input_img_paths1, train_input_img_paths1)
...
img = PIL.ImageOps.autocontrast(keras.preprocessing.image.array_to_img(val_preds[1][:,:,:3]))
display(img)
